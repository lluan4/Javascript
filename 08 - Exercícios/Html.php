...
HTML;
    ...
    public static function getMenuInfos()
    {
       ...
       if (Session::haveRight("ticket", CREATE)) {
        ...
        $conectaGLPI = mysqli_connect("localhost", "root", "");
        mysqli_select_db($conectaGLPI, "glpi");
        $sql = "select count(*) from glpi_ticketsatisfactions S inner join glpi_tickets T on S.tickets_id = T.id inner join glpi_users U on T.users_id_recipient = U.id 
            where T.status = 6 and T.is_deleted = 0 and S.date_begin >= '2020-08-10 00:00:00' and S.date_answered is null and U.id = {$usuarioID}";
        $res = mysqli_query($conectaGLPI, $sql);
        $countSatisfaction = mysqli_fetch_assoc($res);
        $contSatisfacao = (int) end($countSatisfaction);
        $quantidadeLimiteSatisfacao = 0;
